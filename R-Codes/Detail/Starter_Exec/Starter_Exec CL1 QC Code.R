...
dpv <- ""
dpv[1]<- paste0("H:\\01. GIP BAI\\MBR\\Automation\\Automation\\Detail\\Starter_Exec\\CL1_Nation")
dpv[2]<- paste0("H:\\01. GIP BAI\\MBR\\Automation\\Automation\\Detail\\Starter_Exec\\CL1_RM")
...
for(i in 1:length(nat_files))
{
  ...
  a = unique(x)
  b = unique(y)
  b$TERR_NM<- ifelse(grepl("CL1-Cluster",a$TERR_NM ), "US", a$TERR_NM)
  ...
  {
    setwd(err_loc)
    write.csv(a, file = paste0(paste0("Nat1_",i),".csv") , row.names = FALSE)
